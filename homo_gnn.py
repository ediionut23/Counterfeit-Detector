...
def hetero_to_homo(hetero_graph: HeteroData, feature_dims: dict) -> Data:
    ...
    # Copy only tensor/numeric graph-level attributes (strings break DataLoader collate)
    for attr in ['y']:
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Homogeneous GAT Training Script")
    ...
    if args.dataset:
        ...
        patterns = ['*intelligent_pharma*.pt', '*enhanced*.pt', '*pharma*.pt', '*hetero*.pt', '*.pt']
        found = []
        for pat in patterns:
