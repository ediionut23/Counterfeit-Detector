...
#  MMP TRANSFORMATIONS
# ============================================================
from mmp_transformations import MMP_TRANSFORMATIONS as _MMP_FULL
from mmp_transformations import TRANSFORMATION_DESCRIPTIONS
MMP_TRANSFORMATIONS: List[Tuple[str, str, str, str]] = [
    (name, diff, r, p) for (name, diff, cat, r, p, cite) in _MMP_FULL
]
_TRANSFORM_META = {
    name: {"category": cat, "citation": cite}
    for (name, diff, cat, r, p, cite) in _MMP_FULL
}
...
ALLOWED_ATOMS = frozenset({"C", "N", "O", "S", "F", "Cl", "Br", "I", "P", "H", "B"})


def has_only_allowed_atoms(mol: Chem.Mol,
                            allowed: frozenset = ALLOWED_ATOMS) -> bool:
    """True if every atom in mol is in the allowed set."""
    try:
...
class CounterfeitGenerator:
    def __init__(self, seen_smiles: Set[str],
                 target_difficulty_mix: Dict[str, float] = None,
                 target_total: int = 50000,
                 per_transform_cap_factor: float = 1.5):
        """
        Args:
            seen_smiles: global set of canonical SMILES already produced.
            target_difficulty_mix: soft weighting for picking transforms
                by difficulty class.
            target_total: target total number of counterfeits; used to
                compute a per-transformation cap.
            per_transform_cap_factor: cap = (target_total / n_transforms)
                * per_transform_cap_factor. With 21 transforms and
                target=50000, fair share is ~2380; cap_factor=1.5 gives
                max ~3570 per transform. This prevents a single
                permissive transform (e.g. ArH→F) from dominating.
        """
        ...
        fair_share = target_total / max(len(self.transformations), 1)
        self.per_transform_cap = int(fair_share * per_transform_cap_factor)
        self.count_per_transform: Dict[str, int] = defaultdict(int)
        logger.info(f"Per-transformation cap set to {self.per_transform_cap} "
                    f"(fair share {fair_share:.0f} × {per_transform_cap_factor})")
    ...
    def _pick_transformation(self) -> Optional[Transformation]:
        """Pick a transformation that is still below its per-type cap.
        Returns None if every transform is at cap."""
        available = [t for t in self.transformations
                     if self.count_per_transform[t.name] < self.per_transform_cap]
        if not available:
            return None

        by_diff_available = defaultdict(list)
        for t in available:
            by_diff_available[t.difficulty].append(t)
        ...
        for diff, weight in self.difficulty_mix.items():
            cumulative += weight
            if r <= cumulative:
                candidates = by_diff_available.get(diff, [])
                if candidates:
    ...
    def generate_one(self, parent_row: Dict, max_attempts: int = 50) -> Optional[Dict]:
        ...
        for attempt in range(max_attempts):
            ...
            if transform is None:
                # All transforms at cap — this is terminal for this parent
                self.stats["fail_all_at_cap"] += 1
                return None
            ...
            self.count_per_transform[transform.name] += 1

            meta = _TRANSFORM_META.get(transform.name, {})
            return {
...
def build_dataset(total_size: int = 50000,
    ...
    cf_gen = CounterfeitGenerator(seen_smiles=seen, target_total=target_fake)
    ...
    for _, row in tqdm(full_df.iterrows(), total=len(full_df), desc="Graphs"):
        meta = {
            ...
            "category": row.get("category", "authentic"),
    ...
    stats = {
        ...
        "category_distribution": (
            fake_matched["category"].value_counts().to_dict()
            if "category" in fake_matched else {}
        ),
        "citation_distribution": (
            fake_matched["citation"].value_counts().to_dict()
            if "citation" in fake_matched else {}
        ),
        "source_distribution": auth_matched["source"].value_counts().to_dict(),
        "counterfeit_types": (
            fake_matched["counterfeit_type"].value_counts().head(25).to_dict()
