...
from hyp import RobustEnhancedHGTDetector, PyGNativeExplainer, ExplainabilityVisualizer
...
async def lifespan(app: FastAPI):
    ...
    target_model = model_dir / "best_model_f1_0.8176.pt"
...
class AnalyzeResponse(BaseModel):
    html: str
    heatmap_svg: str           
    heatmap_svg_object: str   
    prediction: str
    confidence: float
    charts_features: str
    charts_metrics: str
    charts_probability: str
    charts_hgt_layers: str
    charts_hgt_edges: str
    charts_hgt_agreement: str
...
def _bond_edge_scores(mol: Chem.Mol, edge_importance: dict) -> dict:
    atom_to_type = {a.GetIdx(): a.GetSymbol() for a in mol.GetAtoms()}
    type_counter: dict = {}
    bond_scores: dict = {}
    for bond in mol.GetBonds():
        s, e = bond.GetBeginAtomIdx(), bond.GetEndAtomIdx()
        st, et = atom_to_type[s], atom_to_type[e]
        scores = []
        if st == et:
            key = (st, 'bond_to', st)
            pos = type_counter.get((st, st), 0)
            if key in edge_importance:
                arr = edge_importance[key]
                for p in (2 * pos, 2 * pos + 1):
                    if p < len(arr):
                        scores.append(float(arr[p]))
            type_counter[(st, st)] = pos + 1
        else:
            fwd_key = (st, 'bond_to', et)
            rev_key = (et, 'bond_to', st)
            fwd_pos = type_counter.get((st, et), 0)
            rev_pos = type_counter.get((et, st), 0)
            if fwd_key in edge_importance and fwd_pos < len(edge_importance[fwd_key]):
                scores.append(float(edge_importance[fwd_key][fwd_pos]))
            if rev_key in edge_importance and rev_pos < len(edge_importance[rev_key]):
                scores.append(float(edge_importance[rev_key][rev_pos]))
            type_counter[(st, et)] = fwd_pos + 1
            type_counter[(et, st)] = rev_pos + 1
        if scores:
            bond_scores[bond.GetIdx()] = sum(scores) / len(scores)
    return bond_scores


def generate_2d_svg(mol: Chem.Mol, scores: np.ndarray,
                    bond_scores: dict | None = None) -> str:
    ...
    if bond_scores:
        raw_vals = np.array(list(bond_scores.values()), dtype=float)
        b_min, b_max = raw_vals.min(), raw_vals.max()
        b_range = b_max - b_min + 1e-9

        for bond in mol2d.GetBonds():
            bid = bond.GetIdx()
            if bid not in bond_scores:
                continue
            s_norm = float((bond_scores[bid] - b_min) / b_range)
            if s_norm < 0.1:
                continue
            highlight_bonds.append(bid)
            r_ch = 1.0
            g_ch = 0.75 - 0.75 * s_norm
            b_ch = 0.72 - 0.72 * s_norm
            bond_colors[bid] = (r_ch, g_ch, b_ch)
    else:
        for bond in mol2d.GetBonds():
...
def hgt_charts_to_b64(raw_explanation: dict) -> dict:
    charts: Dict[str, str] = {}
    viz = ExplainabilityVisualizer.__new__(ExplainabilityVisualizer)
    hgt_attention = raw_explanation.get("hgt_attention")
    ...
    def _darken(fig, *axes):
        fig.patch.set_facecolor(_BG)
        for a in axes:
            a.set_facecolor(_BG)
            a.title.set_color(_BRIGHT)
            a.xaxis.label.set_color(_TEXT)
            ...
            if leg:
                ...
                leg.get_frame().set_edgecolor(_BORDER)
                for t in leg.get_texts():
                    t.set_color(_TEXT)
                leg_title = leg.get_title()
                if leg_title:
                    leg_title.set_color(_MUTED)
    ...
    try:
        ...
        viz.plot_layer_wise_trajectories(ax, hgt_attention)
        lo, hi = ax.get_ylim()
        ax.set_ylim(max(0.0, lo), hi)
        _darken(fig, ax)
        charts['hgt_layers'] = _save(fig)
    except Exception as e:
        logger.warning(f"hgt_layers chart: {e}")

    try:
        fig, ax = plt.subplots(figsize=(7, 4))
        viz.plot_edge_type_attention(ax, hgt_attention)
        _darken(fig, ax)
        charts['hgt_edges'] = _save(fig)
    except Exception as e:
        logger.warning(f"hgt_edges chart: {e}")

    try:
        fig, ax = plt.subplots(figsize=(7, 4))
        viz.plot_method_agreement(ax, raw_explanation)
        all_axes = fig.get_axes()
        _darken(fig, *all_axes)
        charts['hgt_agreement'] = _save(fig)
    except Exception as e:
        logger.warning(f"hgt_agreement chart: {e}")
...
function drawDefault() {{
  vw.setStyle({{}}, {{ stick: {{ radius: 0.12, color: '#1e2540' }} }});
  Object.entries(AD).forEach(([serial, d]) => {{
    vw.setStyle({{ serial: parseInt(serial) }}, {{
      sphere: {{ color: d.color, scale: 0.32 }},
      stick:  {{ color: d.color, radius: 0.12 }},
    }});
  }});
  vw.render();
}}
...
async def analyze(request: AnalyzeRequest):
    ...
    obj_raw = raw.get('gnn_object_raw') or {}
    obj_ni = obj_raw.get('node_importance_object', {})
    obj_scores: Dict[int, float] = {}
    for node_type, global_indices in node_type_to_globals.items():
        if node_type in obj_ni:
            imp = obj_ni[node_type]
            for local_idx, global_idx in enumerate(global_indices):
                if local_idx < len(imp):
                    obj_scores[global_idx] = float(imp[local_idx])
    atom_scores_object = np.array([obj_scores.get(i, 0.0) for i in range(mol.GetNumAtoms())])

    edge_grad_raw = raw.get('edge_gradient_raw') or {}
    edge_imp = edge_grad_raw.get('edge_importance', {})
    bond_scores = _bond_edge_scores(mol, edge_imp) if edge_imp else None

    charts = charts_to_b64(raw, mol=mol, atom_scores_norm=atom_scores_norm)
    hgt_charts = hgt_charts_to_b64(raw)
    ...
    return AnalyzeResponse(
        ...
        heatmap_svg=generate_2d_svg(mol, atom_scores, bond_scores=bond_scores),
        heatmap_svg_object=generate_2d_svg(mol, atom_scores_object) if obj_scores else "",
        prediction=prediction,
        confidence=confidence,
        charts_features=charts.get("features", ""),
        charts_metrics=charts.get("metrics", ""),
        charts_probability=charts.get("probability", ""),
        charts_hgt_layers=hgt_charts.get("hgt_layers", ""),
        charts_hgt_edges=hgt_charts.get("hgt_edges", ""),
        charts_hgt_agreement=hgt_charts.get("hgt_agreement", ""),
        method_agreement=float(raw.get("metrics", {}).get("method_agreement", 0.0)),
...
#heatmap-container svg { width: 100% !important; height: auto !important; display: block; }
...
/* Method agreement */
.agreement-row { display: flex; align-items: center; gap: 10px; margin-top: 10px; }
.agreement-label { font-size: 10px; color: var(--muted); white-space: nowrap; }
.agreement-bar-wrap { flex: 1; height: 4px; background: rgba(255,255,255,.08); border-radius: 2px; position: relative; }
.agreement-bar { height: 100%; border-radius: 2px; transition: width .5s ease; }
.agreement-val { font-size: 11px; font-family: 'JetBrains Mono', monospace; white-space: nowrap; }
...
<div id="loading" style="display:none">
  ...
  <div class="loading-steps">
    ...
    <div class="step"><div id="st3" class="step-dot"></div><span>Extracting native HGT attention</span></div>
    <div class="step"><div id="st4" class="step-dot"></div><span>Computing gradient importance</span></div>
    <div class="step"><div id="st5" class="step-dot"></div><span>Generating visualizations</span></div>
  </div>
</div>
    ...
    <div class="results-scroll">
      ...
      <div class="r-card">
        ...
        <div class="chart-tabs" style="margin-bottom:8px">
          <button class="tab-btn active" onclick="showHeatmap('attributes')" id="tab-hm-attr">Features avg</button>
          <button class="tab-btn" onclick="showHeatmap('object')" id="tab-hm-obj">Node-level</button>
        </div>
        <div id="heatmap-container"></div>
        <div style="font-size:9px;color:var(--muted);margin-top:6px;line-height:1.5" id="hm-legend">
          <b>Features avg</b>: mean of GNNExplainer attribute masks per atom &nbsp;·&nbsp;
          <b>Node-level</b>: native per-atom scalar (Calea 2)
        </div>
      </div>
      ...
      <!-- HGT Native Attention -->
      <div class="r-card">
        <div class="r-card-title">HGT Native Attention</div>
        <div class="chart-tabs">
          <button class="tab-btn active" onclick="showHgtChart('hgt_edges')" id="tab-hgt_edges">Edge Types</button>
          <button class="tab-btn" onclick="showHgtChart('hgt_agreement')" id="tab-hgt_agreement">Agreement</button>
        </div>
        <img id="hgt-chart-img" src="" alt="HGT Chart" style="max-width:100%;border-radius:8px;margin-top:8px;">
      </div>
...
let currentHgtCharts = {};
let activeHgtChart = 'hgt_edges';
let currentHeatmaps = {};
let activeHeatmap = 'attributes';
...
async function showLoading() {
  ...
  [1,2,3,4,5].forEach(i => { const d = document.getElementById('st' + i); if (d) d.className = 'step-dot'; });
...
function showResults(data) {
  ...
  // 2D Heatmap — store both and show active
  currentHeatmaps = {
    attributes: data.heatmap_svg,
    object: data.heatmap_svg_object || '',
  };
  ...
  // HGT attention charts
  currentHgtCharts = {
    hgt_layers:    data.charts_hgt_layers,
    hgt_edges:     data.charts_hgt_edges,
    hgt_agreement: data.charts_hgt_agreement,
  };
  showHgtChart(activeHgtChart);
...
function showHeatmap(type) {
  activeHeatmap = type;
  ['attributes', 'object'].forEach(t => {
    const btn = document.getElementById('tab-hm-' + (t === 'attributes' ? 'attr' : 'obj'));
    if (btn) btn.className = 'tab-btn' + (t === type ? ' active' : '');
  });
  const container = document.getElementById('heatmap-container');
  const svg = currentHeatmaps[type];
  if (svg) {
    container.innerHTML = svg;
  } else {
    container.innerHTML = '<p style="color:var(--muted);font-size:11px;padding:8px 0">Not available — GNNExplainer object pass did not produce scores.</p>';
  }
}
...
function showHgtChart(type) {
  activeHgtChart = type;
  ['hgt_edges','hgt_agreement'].forEach(t => {
    const btn = document.getElementById('tab-' + t);
    if (btn) btn.className = 'tab-btn' + (t === type ? ' active' : '');
  });
  const img = document.getElementById('hgt-chart-img');
  const src = currentHgtCharts[type];
  img.src = src ? ('data:image/png;base64,' + src) : '';
  img.style.display = src ? 'block' : 'none';
}
