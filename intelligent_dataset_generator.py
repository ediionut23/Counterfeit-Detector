...
class CounterfeitGenerator:
    ...
    def generate_one(self, parent_row: Dict, max_attempts: int = 20) -> Optional[Dict]:
        ...
        for attempt in range(max_attempts):
            ...
            # On larger molecules (15+ heavy atoms), single-atom changes give
            # high similarity, so we raise the floor to ensure counterfeits
            # are genuinely close to the parent.
            sim = _tanimoto(parent_mol, product)
            if sim < 0.50 or sim > 0.98:
                continue
...
def build_dataset(total_size: int = 50000,
    ...
    fetcher = AuthenticFetcher(max_per_scaffold=20)
    chembl_rows = fetcher.fetch_chembl(int(target_auth * 0.7))
    pubchem_rows = fetcher.fetch_pubchem(int(target_auth * 0.6))
    ...
    # Target: overgenerate counterfeits — we lose many in property matching
    target_fake = int(half * 2.0)

    # Generate up to 5 counterfeits per parent for higher yield
    ...
    with tqdm(total=target_fake, desc="Counterfeits") as pbar:
        for parent in parents:
            if len(counterfeit_rows) >= target_fake:
                break
            # Up to 5 counterfeits per parent
            for _ in range(5):
                if len(counterfeit_rows) >= target_fake:
                    break
                cf = cf_gen.generate_one(parent, max_attempts=30)
                if cf is not None:
